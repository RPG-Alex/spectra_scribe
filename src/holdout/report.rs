...
pub fn class_distribution_report<H: Holdout>(holdout: &H) -> Vec<ClassDistribution> {
    holdout.class_indices().iter().map(|&class_index| {
        let train_positive = holdout.train_dataset().samples().iter().filter(|sample| sample.element_present[class_index]).count();

        let validation_positive = holdout.validation_dataset().samples().iter().filter(|sample| sample.element_present[class_index]).count();
