...
pub fn aggregate_metrics(metrics: &[ElementMetrics]) -> AggregateMetrics {
    ...
    let tp = metrics.iter().map(|m| m.true_positive as f64).sum::<f64>();
    let tn = metrics.iter().map(|m| m.true_negative as f64).sum::<f64>();
    let fp = metrics.iter().map(|m| m.false_positive as f64).sum::<f64>();
    let fne = metrics.iter().map(|m| m.false_negative as f64).sum::<f64>();

    let micro_precision = safe_div(tp, tp+fp);
    let micro_recall = safe_div(tp, tp+fne);
    let micro_f1 = safe_div(2.0 * micro_precision * micro_recall, micro_precision+micro_recall);
